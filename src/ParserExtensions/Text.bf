...
public static
{
	...
	{
		var num = p.Number();
		return num.HasValue? (.)Math.Round(num.Value) : (int?)null;
	}
	...
	public static StringView? QuotedText(this Parser p, char8 quoteChar, bool singleLine, params StringView[] ignoredSubstrings)
	{
		p.Try!();
			...
			while(true)
			{
				...
				if(p.Char().Exists(let ch))
				{
					if(ch == quoteChar)
						...
						p.endTry!<StringView>(p.addTextError!("This quotation is not suited for multiple lines"));
				}
				else
				{
